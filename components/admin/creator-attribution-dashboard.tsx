...
import { ArrowUpRight, Bot, CircleDollarSign, CreditCard, Download, MousePointerClick, RefreshCcw, ShoppingCart, UserPlus, UsersRound } from 'lucide-react'
  ...
  return (
    <section>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <SummaryMetric label="Paywall Reached" value={formatNumber(overall.paywallViews)} detail={`${formatPercent(overall.paywallViews, overall.installs)} of installs`} icon={CreditCard} />
...
function AttributionReportFrame({ eyebrow, title, description, report }: { eyebrow: string; title: string; description: string; report: AdminAttributionReport }) {
  const funnel = [
    ...
    { label: 'Paywall', value: report.paywallViews, icon: CreditCard },
  ...
  return <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-4 sm:p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-400">{eyebrow}</p><h3 className="mt-2 text-lg font-semibold tracking-[-0.03em]">{title}</h3><p className="mt-1 text-xs leading-5 text-zinc-500">{description}</p><div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5"><ReportMetric label="Link Clicks" value={formatNumber(report.clicks)} detail={`${formatNumber(report.uniqueClicks)} unique`} /><ReportMetric label="Installs" value={formatNumber(report.installs)} detail={`${formatPercent(report.installs, report.clicks)} of clicks`} /><ReportMetric label="Paywall Reached" value={formatNumber(report.paywallViews)} detail={`${formatPercent(report.paywallViews, report.installs)} of installs`} /><ReportMetric label="Paid Customers" value={formatNumber(report.paidCustomers)} detail={`${formatPercent(report.paidCustomers, report.paywallViews)} of paywalls`} /><ReportMetric label="Net Revenue" value={formatMoney(report.revenueCents)} detail={`${formatMoney(report.grossRevenueCents)} gross`} /></div><div className="mt-5 overflow-x-auto rounded-xl bg-zinc-50 p-3"><div className="flex min-w-[640px] items-center">{funnel.map((step, index) => { const Icon = step.icon; return <div key={step.label} className="contents"><div className="min-w-0 flex-1 text-center"><span className="mx-auto grid size-8 place-items-center rounded-lg bg-white shadow-sm"><Icon className="size-3.5 text-zinc-500" /></span><p className="mt-2 text-base font-semibold">{formatNumber(step.value)}</p><p className="text-[10px] font-medium text-zinc-400">{step.label}</p></div>{index < funnel.length - 1 ? <ArrowUpRight className="size-3.5 shrink-0 rotate-45 text-zinc-300" /> : null}</div> })}</div></div><div className="mt-4 grid gap-2 sm:grid-cols-3"><DetailCard label="Last 30 Days" value={`${formatNumber(report.recentClicks)} clicks`} detail={`${formatNumber(report.recentInstalls)} installs · ${formatNumber(report.recentPaywallViews)} paywalls · ${formatNumber(report.recentPaidCustomers)} paid · ${formatMoney(report.recentRevenueCents)} net`} icon={RefreshCcw} /><DetailCard label="First-Touch Credit" value={`${formatNumber(report.firstTouchPaidCustomers)} paid`} detail={`${formatNumber(report.firstTouchSignups)} signups · ${formatNumber(report.firstTouchPaywallViews)} paywalls · ${formatMoney(report.firstTouchRevenueCents)} net revenue`} icon={UserPlus} /><DetailCard label="Data Quality" value={`${formatNumber(report.botClicks)} bot clicks`} detail={`${formatNumber(report.refunds)} refunds · ${formatNumber(report.disputes)} disputes · ${formatMoney(report.reversedRevenueCents)} reversed`} icon={Bot} /></div></section>
}
...
function emptyReport(): AdminAttributionReport { return { clicks: 0, uniqueClicks: 0, botClicks: 0, signups: 0, installs: 0, paywallViews: 0, checkouts: 0, purchases: 0, paidCustomers: 0, refunds: 0, disputes: 0, grossRevenueCents: 0, reversedRevenueCents: 0, revenueCents: 0, firstTouchSignups: 0, firstTouchPaywallViews: 0, firstTouchPaidCustomers: 0, firstTouchRevenueCents: 0, recentClicks: 0, recentSignups: 0, recentInstalls: 0, recentPaywallViews: 0, recentPaidCustomers: 0, recentRevenueCents: 0 } }
