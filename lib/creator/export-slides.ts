...
async function drawSlideFrame(ctx: CanvasRenderingContext2D, slide: ContentSlide, image: HTMLImageElement | null, source: GeneratorImage | undefined, width: number, height: number, timeMs: number) {
  ...
  if (source?.landmarks) {
    ...
    drawOverlay(ctx, overlay.primitives, width, height, timeMs)
...
function drawTemplate(ctx: CanvasRenderingContext2D, slide: ContentSlide, width: number, height: number, timeMs: number) {
  ...
  drawHeader(ctx, slide.eyebrow, slide.templateId, margin, width, height, enter(timeMs, 580, 520))
...
function drawScoreRows(ctx: CanvasRenderingContext2D, slide: ContentSlide, width: number, height: number, timeMs: number) {
  ...
  rows.forEach((row, index) => withEnter(ctx, enter(timeMs, 920 + index * 90, 480), () => { const rowY = rowStart + rowHeight * index; ctx.fillStyle = '#fff'; ctx.font = `500 ${width * .022}px Arial`; ctx.fillText(row.label, x + width * .04, rowY); const barX = x + width * .34, barWidth = boxWidth * .49; ctx.fillStyle = 'rgba(255,255,255,.18)'; roundedRect(ctx, barX, rowY, barWidth, height * .007, height * .004); ctx.fill(); ctx.fillStyle = '#fff'; roundedRect(ctx, barX, rowY, barWidth * scoreRatio(row.value), height * .007, height * .004); ctx.fill(); ctx.textAlign = 'right'; ctx.font = `600 ${width * .021}px monospace`; ctx.fillText(displayScore(row.value), x + boxWidth - width * .04, rowY - height * .004); ctx.textAlign = 'left' }))
}

function drawCta(ctx: CanvasRenderingContext2D, slide: ContentSlide, width: number, height: number, timeMs: number) {
  const x = width * .07
  withEnter(ctx, enter(timeMs, 820, 520), () => { ctx.fillStyle = '#fff'; ctx.fillRect(x, height * .62, width * .11, width * .11); ctx.fillStyle = '#000'; ctx.textAlign = 'center'; ctx.font = `900 ${width * .052}px monospace`; ctx.fillText('M', x + width * .055, height * .637); ctx.textAlign = 'left' })
  withEnter(ctx, enter(timeMs, 1000, 520), () => { ctx.fillStyle = '#fff'; ctx.font = `600 ${Math.min(width * .075, height * .06)}px Arial`; let y = height * .72; wrapText(ctx, slide.headline, width * .84, ctx.font).forEach((line) => { ctx.fillText(line, x, y); y += width * .07 }) })
  withEnter(ctx, enter(timeMs, 1320, 520), () => { ctx.fillStyle = '#fff'; ctx.fillRect(x, height * .89, width * .4, height * .055); ctx.fillStyle = '#000'; ctx.font = `600 ${width * .022}px monospace`; ctx.fillText('OPEN MOGGING  →', x + width * .035, height * .905) })
}
...
function drawOverlay(ctx: CanvasRenderingContext2D, primitives: ResolvedPrimitive[], width: number, height: number, timeMs: number) {
  primitives.forEach((primitive) => {
    ...
    if (primitive.kind === 'label') { drawTag(ctx, primitive.title, Math.max(12, Math.min(width - width * .24, primitive.point.x + (1 - progress) * (primitive.align === 'right' ? 42 : -42))), Math.max(12, Math.min(height - height * .08, primitive.point.y)), Math.max(18, width * .014), primitive.value, progress); return }
