...
function SubmitContent() {
  ...
  async function submit() {
    ...
    try {
      ...
      await apiPost('/api/creator/submissions', { title, platform, caption: caption || null, postUrl: postUrl || null, videoUrl: intent.publicUrl, videoStorageKey: intent.key, videoContentType: file.type, videoSizeBytes: file.size })
  ...
  if (!data?.profile) {
    return <><CreatorHeader eyebrow="New submission" title="Submit a video" description="Finish your payout information once, then return here to send your first video." /><div className="rounded-[28px] border border-dashed border-zinc-300 bg-white p-10 text-center"><p className="text-sm font-medium">Creator profile required</p><p className="mt-2 text-sm text-zinc-500">Add your creator identity and payout preference before uploading.</p><Button asChild className="mt-6 h-11 rounded-xl"><Link href="/creator/payout-information">Set up payout information</Link></Button></div></>
  }
      ...
      <div className="t-page-slide min-h-[720px] sm:min-h-[650px]" data-page={step}>
        <section className="t-page" data-page-id="1">
          <form onSubmit={review} className="grid gap-6 rounded-[28px] border border-zinc-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.05)] sm:p-8">
            <div className="grid gap-5 sm:grid-cols-2"><Field label="Video title"><input className={fieldClass} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Summer campaign — cut 01" required /></Field><Field label="Platform"><select className={fieldClass} value={platform} onChange={(event) => setPlatform(event.target.value)}>{['TikTok', 'Instagram Reels', 'YouTube Shorts', 'YouTube', 'Other'].map((item) => <option key={item}>{item}</option>)}</select></Field></div>
        ...
        <section className="t-page" data-page-id="2">
          <div className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.05)] sm:p-8">
            ...
            <div className="mt-7 grid gap-3 rounded-2xl bg-zinc-50 p-4 text-sm"><ReviewRow label="File" value={file?.name || ''} /><ReviewRow label="Size" value={file ? formatBytes(file.size) : ''} /><ReviewRow label="Payout" value={data.profile.paymentOption === 'paypal' ? 'PayPal' : data.profile.cryptoNetwork || 'Crypto'} /><ReviewRow label="Status" value="Pending review" /></div>
...
function SubmissionGuidance() {
  const requirements = [
    'Submit content from an approved connected account',
  ...
  return (
    <div className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-5">
        ...
        <p className="mt-4 text-sm leading-6 text-zinc-600">Submit only when you’re satisfied with the post’s performance. The view count used during review becomes the payout snapshot and <strong className="font-semibold text-black">will not update afterward.</strong></p>
      </section>
    </div>
  )
}
