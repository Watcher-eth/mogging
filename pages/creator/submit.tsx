...
function SubmitContent() {
  ...
  async function submit() {
    if (!analyticsScreenshot || !formatId || !postUrl || !requirementsConfirmed) return
    setSubmitting(true)
    try {
      ...
      await apiPost('/api/creator/submissions', { formatId, requirementsConfirmed: true, socialAccountId: socialAccountId || null, postUrl, analyticsScreenshotUrl: intent.publicUrl, analyticsStorageKey: intent.key, analyticsContentType: analyticsScreenshot.type, analyticsSizeBytes: analyticsScreenshot.size })
      ...
      <div className="t-page-slide min-h-[720px] sm:min-h-[650px]" data-page={step}>
        ...
        <section className="t-page" data-page-id="2">
          <div className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.05)] sm:p-8">
            ...
            <div className="mt-8 flex flex-col-reverse gap-2 sm:flex-row sm:justify-between"><Button variant="ghost" className="h-11 rounded-xl" onClick={() => setStep(1)} disabled={submitting}><ChevronLeft />Back to edit</Button><Button className="h-11 rounded-xl px-5" onClick={() => void submit()} disabled={submitting || !requirementsConfirmed}>{submitting ? <Loader2 className="animate-spin" /> : <Check />} {submitting ? 'Uploading analytics…' : 'Submit video'}</Button></div>
          </div>
        </section>
      </div>
